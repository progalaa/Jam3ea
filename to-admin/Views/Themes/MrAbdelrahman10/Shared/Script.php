...
<?php if (($pAction === 'Index' || $pAction === 'Details') && ($dButtons === true)) { ?>
            ...
            $("#data").sortable({
                stop: function (e, ui) {
                    $('#updateSort').show();
                    _SortingItems = $.map($(this).find('tr'), function (el) {
            ...
            $('#updateSort').click(function () {
                $.ajax({
                    url: '<?php echo ADM_BASE . $pID; ?>/UpdateSorting',
                    type: 'post',
                    data: {Sort: _SortingItems},
                    beforeSend: function () {
                        $("#updateSortDiv").html('<img src="<?php echo ADM_CURRENT_URL_TEMPLATE ?>img/ajax-loaders/ajax-loader-6.gif" />');
...
<?php } ?>
        ...
        $('#cbPagination').unbind('change').click(function () {
            var _count = $(this).val();
            var _Data = "rowsNum=" + _count;
            $.ajax({url: '<?php echo FullCurrentUrl() ?>',
                type: 'post',
                data: _Data,
                beforeSend: function () {
                }, success: function () {
                    Redirect('<?php echo FullCurrentUrl() ?>');
                }, complete: function () {
                }, error: function (xhr, ajaxOptions, thrownError) {
                    DoError();
                }});
