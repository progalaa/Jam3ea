...
function RenderHTMLNavDB($_pages, $base_url = SITE_URL) {
    $str = '';
    if ($_pages) {
        $str .='<ul class="pagination pagination-md">';
        foreach ($_pages as $i) {
            $str .= '<li>';
            $str .= Anchor($i['Url'] != 'javascript:void(0);' ? $base_url . $i['Url'] : 'javascript:void(0);', $i['Title'], '', $i['Ajax']);
            $str .= '</li>';
        }
        $str .='</ul>';
    }
...
function InputBox($Name, &$Value, $PlaceHolder = '', $Attributes = '', $Default = '', $Type = 'text') {
...
function InputAutoComplete($ID, $vID, $Url, &$hValue, &$vValue, $PlaceHolder = '', $Attributes = '', $Default = '') {
    return
            InputBox($vID, $vValue, $PlaceHolder, $Attributes, $Default) .
...
function Img($Value, $Attributes = '') {
    return '<img src="' . $Value . '" ' . $Attributes . ' />';
}
...
function ImageAjaxUpload(&$PictureValue, $Url, &$errPicture, $PictureID = 'Picture', $ImageID = 'Image') {
    return Anchor('#FileManager', Img(GetImageThumbnail(GetValue($PictureValue)), 'class="img-polaroid img-up" data-toggle="modal" id="' . $ImageID . '"'), 'data-toggle="modal"', false) . '
